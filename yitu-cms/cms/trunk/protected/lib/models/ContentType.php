 ...
 */
class ContentType extends YituActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('name', 'required'),
			array('language, ctime, mtime, status', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>16),
			array('logo', 'length', 'max'=>36),
			array('template', 'length', 'max'=>20),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, language, name, logo, template, ctime, mtime, status', 'safe', 'on'=>'search'),
		);
	}
