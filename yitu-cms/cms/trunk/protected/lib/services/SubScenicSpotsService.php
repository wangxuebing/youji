...
class SubScenicSpotsService extends BaseService {
    ...
    public function addData($data, &$status) {
        ...
        if ($res) {
            ...
            Yii::log("Add item to SubScenicSpots success, data is {$msg}", CLogger::LEVEL_INFO);
        } else {
            $status = -2;
            $msg = json_encode($data);
            $error = json_encode($obj->getErrors());
            Yii::log("Add item to SubScenicSpots failed, data is {$msg}, error is {$error}", CLogger::LEVEL_ERROR);
        }
    ...
    public function updateData($data) {
        ...
        $obj = SubScenicSpots::model()->findByAttributes($where);
        if (!$obj) {
            return false;
        }
	...
	public function getAllData() {
		$objs = SubScenicSpots::model()->findAllByAttributes(array('status'=>self::STATUS_NORMAL, 'language'=>$this->language));
        $data = array();
        foreach ($objs as $obj) {
            $data[] = $this->parseData($obj);
        }
	...
	protected function parseData($obj) {
        ...
        $type = $service->getDataByPkFromCache($data['type_id']);
        if ($type) {
