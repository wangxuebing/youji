...
class ResourceTypeService {
    ...
    public function addData($data, &$status) {
        ...
        $attri_maps = ResourceType::model()->attributeLabels();
        ...
        $obj = ResourceType::model()->findByAttributes(
                    $map
                );
        if ($obj) {
            $status = -1;
            $msg = json_encode($data);
            Yii::log("Add item to ResourceType repeated, data is {$msg}", CLogger::LEVEL_ERROR);
            return $obj['id'];
        }
        ...
        if ($res) {
            $msg = json_encode($data);
            Yii::log("Add item to ResourceType success, data is {$msg}", CLogger::LEVEL_ERROR);
        } else {
            $status = -2;
            $msg = json_encode($data);
            Yii::log("Add item to ResourceType failed, data is {$msg}", CLogger::LEVEL_INFO);
        }
    ...
    public function delDataByField($field) {
        if (!is_array($field) || $field == false) {
            ...
            $res = ResourceType::model()->deleteAllByAttributes($field);
            if (!$res) {
                $msg = json_encode($field);
                Yii::log("Delete item from ResourceType failed, condition is {$msg}", CLogger::LEVEL_ERROR);
            }
            return $res;
        }
    }
    ...
    public function updateData($data) {
        ...
        $obj = ResourceType::model()->findByPk($data['id']);
        if (!$obj) {
            return false;
        }
        ...
        if (!$res) {
            $msg = json_encode($data);
            Yii::log("Update item to ResourceType failed, data is {$msg}", CLogger::LEVEL_ERROR);
        }
    ...
    public function getDataByField($data) {
        ...
        $objs = ResourceType::model()->findAllByAttributes(
                    $where
                );
	...
	public function getDataByPk($id) {
        $obj = ResourceType::model()->findByPk($id);
        if ($obj) {
	...
	public function getAllData() {
		$objs = ResourceType::model()->findAll();
        $data = array();
        foreach ($objs as $obj) {
            $data[] = $this->parseData($obj);
        }
