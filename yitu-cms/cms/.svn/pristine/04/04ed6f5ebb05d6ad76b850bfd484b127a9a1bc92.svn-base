 ...
 */
class ResourceType extends ResourceActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('name, domain, postfix', 'required'),
			array('parent_id, status', 'numerical', 'integerOnly'=>true),
			array('name, domain', 'length', 'max'=>16),
			array('postfix', 'length', 'max'=>64),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, name, domain, postfix, parent_id, status', 'safe', 'on'=>'search'),
		);
	}
