...
class SpotsController extends Controller {
     ...
     */
     public function actionCreateAppendix() {
         ...
         if ($id == 0 || $type_id == 0 || $title == '' || count($content) <= 0) {
             $this->_data['error_code'] = self::STATUS_PARAMS_ERROR;
         } else {
             //create the content meta
             $default_language = isset($_REQUEST['language']) ? $_REQUEST['language'] : 1;
             $meta = array();
             $meta['title'] = $title;
             ...
             if ($status == -1) {
                 ...
                 foreach ($languages as $l) {
                     if ($l != $default_language) {
                        $meta['language'] = $l;
                        $status = 0;
                        $meta_service->addData($meta, $status);
                     }
                 }
             }
         }
     ...
     */
     public function actionCreateIntroduction() {
         ...
         $params = json_encode($_REQUEST);
         Yii::log("Create introduction params is {$params}", CLogger::LEVEL_INFO);
     ...
     */
     public function actionUpdateIntroduction() {
         ...
         $params = json_encode($_REQUEST);
         Yii::log("Update introduction params is {$params}", CLogger::LEVEL_INFO);
     ...
     */
     ...
     public function actionCreateSubScenicSpots() {
         ...
         $params = json_encode($_REQUEST);
         Yii::log("Create sub spots params is {$params}", CLogger::LEVEL_INFO);
                         ...
                         if ($meta['import_flag'] == 1) { //add existed meta
                             ...
                             foreach ($languages as $l) {
                                 if ($l != $default_language) {
                                     $obj['language'] = $l;
                                     $status = 0;
                                     $meta_service->addData($data, $status);
                                 }
                             }
                         }
     ...
     public function actionUpdateSubScenicSpots() {
         ...
         $params = json_encode($_REQUEST);
         Yii::log("Update sub spots params is {$params}", CLogger::LEVEL_INFO);
         ...
         if ($id == 0) {
             ...
             if (!$spots) {
                $this->_data['error_code'] = self::STATUS_PARAMS_ERROR;
                 ...
                 //delete scenic spots content link, all languages
                 $content_link_service = new ScenicSpotsContentLinkService();
                 $languages = array_keys(Yii::app()->c->spots['language']);
                 ...
                 //delete sub spots meta, all languages
                 $meta_service = new ContentMetaService();
                 ...
                 //delete the sub spots, all languages
                 $where = array('id'=>$id);
                 foreach ($languages as $l) {
                     $where['language'] = $l;
