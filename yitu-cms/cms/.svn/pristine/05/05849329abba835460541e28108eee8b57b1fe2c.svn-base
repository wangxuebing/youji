...
class ParseUtil {
        ...
        if ($content != null) {
            ...
            foreach ($content as $item) {
                if ($item['type'] == 0) { //text
                    ...
                    $md = $item['content'];
                    $item['content'] = self::getResourceUrlByTypeAndMd($type, $item['content']);
                    $item['face'] = '';
                } else if ($item['type'] == 2) { //video
                    $type = $resource_map[$item['type']];
                    $md = $item['content'];
                    $item['content'] = self::getResourceUrlByTypeAndMd($type, $item['content']);
                    $item['face'] = self::getResourceUrlByTypeAndMd('image', $item['face_md']);;
    ...
    public static function getResourceUrlByTypeAndMd($type, $md) {
        ...
        $url['url'] = $service->getResourceUrl($type, $md);
        $url['md'] = $md;
        return $url;
    }
