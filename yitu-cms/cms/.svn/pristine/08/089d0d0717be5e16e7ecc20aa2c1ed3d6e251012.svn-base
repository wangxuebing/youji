...
class ScenicSpotsService extends BaseService {
    ...
    public function getDataByPk($id) {
        ...
        $obj = ScenicSpots::model()->findByAttributes($where);
        if ($obj) {
    ...
    //CMS创建景点时使用
    public function getDataByFieldWithoutLanguage($data) {
        if (count($data) <= 0 || !is_array($data)) {
            return array();
        }
        $attri_maps = ScenicSpots::model()->attributeLabels();
    ...
    public function getAllData() {
        $objs = ScenicSpots::model()->findAllByAttributes(array('status'=>0, 'language'=>$this->language));
        $data = array();
        foreach ($objs as $obj) {
            $data[] = $this->parseData($obj);
        }
    ...
    public function updateData($data) {
        ...
        if (!$res) {
            $msg = json_encode($data);
            $error = json_encode($obj->getErrors());
            Yii::log("Update item to ScenicSpots failed, data is {$msg}, error is {$error}", CLogger::LEVEL_ERROR);
        }
        return $res;
    }
    ...
    public function getNearBySpots($lng, $lat, $distance=2) {
        ...
        $sql = "select * from `scenic_spots` where language={$this->language} and latitude<>0 and latitude>{$squares['right-bottom']['lat']} and latitude<{$squares['left-top']['lat']} and longitude>{$squares['left-top']['lng']} and longitude<{$squares['right-bottom']['lng']}";
    ...
    protected function parseData($obj, $detail_flag=false) {
        ...
        if ($data['content_face'] != '') {
            $data['content_face'] = ParseUtil::getResourceUrlByTypeAndMd('image', $data['content_face']);
        }
