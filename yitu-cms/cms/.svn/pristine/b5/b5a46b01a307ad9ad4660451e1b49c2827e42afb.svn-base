$(function(){
  ...
  var taskListErrorHtml = '<li class="list_error">{0}</li>';
  ...
  tokenOverdue = function(r){
    if(r.error_code == 30001){
      ...
      var html = taskListErrorHtml.format(r.error_msg);
      $('#taskslist ul').html(html);      
    }
  };
  ...
  taskList = function(){
    ...
    $.ajax({
      ...
      jsonp: "callback",
      beforeSend: function(){
        html = taskListErrorHtml.format('正在加载，请稍候…');
        $('#taskslist ul').html(html);
      },
  ...
  $('#search_tasks').click(function(){
    ...
    if(kw == ''){
      ...
      $.ajax({
        ...
        jsonp: "callback",
        beforeSend: function(){
          var html = taskListErrorHtml.format('正在搜索，请稍候…');
          $('#taskslist ul').html(html);
        },
        ...
        error: function(searchtasks){
          var html = taskListErrorHtml.format('搜索失败');
          $('#taskslist ul').html(html);
        }
      });
    }
  ...
  $('#taskslist').delegate('.languages select','change',function(){
    ...
    selectd != 0 ? $(this).parent('.languages').siblings('a').removeClass('disabled') : $(this).parent('.languages').siblings('a').addClass('disabled');
  });
